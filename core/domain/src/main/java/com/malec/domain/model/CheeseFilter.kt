...
data class CheeseFilter(
    ...
    val sortBy: CheeseSort? = CheeseSort.ID
) {
    companion object {
        val empty = CheeseFilter()
    }
}

fun List<Cheese>.filteredBy(filter: CheeseFilter): List<Cheese> {
    var filteredList = toList().filter { !it.isDeleted }
    ...
    filteredList = when (filter.sortBy) {
        ...
        CheeseSort.DATE_END -> filteredList.sortedByDescending { it.date }
