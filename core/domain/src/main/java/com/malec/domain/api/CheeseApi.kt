...
import com.malec.domain.model.CheeseF
import com.malec.domain.model.CheeseFilter
import com.malec.domain.model.filteredBy
...
class CheeseApi(private val ref: ReferenceAllocator) {
    ...
    suspend fun getAll(): List<Cheese> {
        ...
        return cheeses.filterNotNull().sortedByDescending { it.id }
    }

    suspend fun getAllFilteredFlow(filter: CheeseFilter): Flow<List<Cheese>> {
